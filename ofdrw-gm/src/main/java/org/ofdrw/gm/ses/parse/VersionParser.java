...
public class VersionParser {
    ...
    public static SESVersionHolder parseSES_SignatureVersion(Object o) {
        ASN1Sequence seq = ASN1Sequence.getInstance(o);
        ASN1Sequence tbsSignSeq = (ASN1Sequence)seq.getObjectAt(0);
        ASN1Integer asn1Integer = (ASN1Integer) tbsSignSeq.getObjectAt(0);
        Integer sesversion = asn1Integer.getValue().intValue();
        ...
        if (seq.size() >= 4 && seq.size() <= 5) {
            ...
            //如果是3，为中安的一个v3定制版本。操蛋的版本
            if(sesversion == 3){
                version = SESVersion.v3;
            }else {
                /*
