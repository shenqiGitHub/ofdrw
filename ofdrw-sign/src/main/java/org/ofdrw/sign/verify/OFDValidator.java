...
import org.ofdrw.sign.verify.container.SESV3ValidateContainer;
...
public class OFDValidator implements Closeable {
     ...
     */
    public void exeValidate() throws IOException, GeneralSecurityException {
        exeValidate(null);
    }

    /**
     *
     * @param signFileLocStr  指定一个signnature xml的相对路径
     * @throws OFDVerifyException       验证异常，电子签名失效
     * @throws DocNotSignException      文件未进行电子签名
     * @throws IOException              文件读写过程中IO异常
     * @throws NoSuchAlgorithmException 未知的杂凑算法
     */
    public void exeValidate(String signFileLocStr) throws IOException, GeneralSecurityException {
        rl.save();
        try {
            rl.cd("/");
            // 获取电子签名列表文件路径
            ...
            for (org.ofdrw.core.signatures.Signature sigRecord : signatures) {
                ...
                // 获取 Signature.xml 文件路径,用于进行指定的图章的签章验证
                ST_Loc signFileLoc = sigRecord.getBaseLoc();
                if(signFileLocStr != null && !signFileLoc.getLoc().equals(signFileLocStr)){
                    continue;
                }
                ...
                try {
                    ...
                    final byte[] sesSignatureBin = Files.readAllBytes(signedValueFilePath);
                    //设置容器版本
                    setValidatorVersion(sesSignatureBin);
    ...
    public List<SESChainData> getSignChainData() throws IOException, GeneralSecurityException {
        List<SESChainData> list = new ArrayList<>();
        rl.save();
        try {
            rl.cd("/");
            // 获取电子签名列表文件路径
            ...
            for (org.ofdrw.core.signatures.Signature sigRecord : signatures) {
                ...
                try {
                    ...
                    SESChainData sesChainData = validator.getSignCert(Files.readAllBytes(signedValueFilePath));
                    sesChainData.setSignFileLocStr(signFileLoc.getLoc());
                    list.add(sesChainData);
                } finally {
                    rl.restore();
                }
            }
    ...
    /**
     * 设置核验容器的版本
     * @param sesSignatureBin 签名值的字节码
     */
    private void setValidatorVersion(byte[] sesSignatureBin) throws OFDVerifyException {
        SESVersionHolder v = VersionParser.parseSES_SignatureVersion(sesSignatureBin);
        if (v.getVersion() == SESVersion.v4) {
            setValidator(new SESV4ValidateContainer());
        }else if(v.getVersion() == SESVersion.v3) {
            setValidator(new SESV3ValidateContainer());
        }else if(v.getVersion() == SESVersion.v1) {
            setValidator(new SESV1ValidateContainer());
        }else {
